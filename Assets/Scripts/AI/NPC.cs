...
public class NPC : Interactable {
	...
	public override void Interact(){
		LoadDialogue ();
		if (questGiver == true) {
			dialogSystem.AddNewDialogue (dialogue, objectName, questionLine, questID);
		} else if (Terminal == true) {
			dialogSystem.AddNewDialogue (dialogue, objectName, questionLine);
		}
	}
		...
		if (questGiver == true) {
			...
			for (int i = 0; i < QuestManager.questManager.questList.Count; i++){
				if (QuestManager.questManager.questList[i].id == questID && QuestManager.questManager.questList[i].progess == Quest.QuestProgress.AVAILABLE) {
					...
					questionLine = 10;
				}  else if(QuestManager.questManager.questList[i].id == questID && QuestManager.questManager.questList[i].progess == Quest.QuestProgress.DONE){
					for (int o = 0; o < DatabaseLoader.database.DialogueData [objectName.ToString ()] [0] ["questDone"].Count; o++) {
						dialogue.Add ((string)DatabaseLoader.database.DialogueData [objectName.ToString ()] [0] ["questDone"] [o]);
					}
					questionLine = 10;
				}
			}
